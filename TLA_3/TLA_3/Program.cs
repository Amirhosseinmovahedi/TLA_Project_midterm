...
public class Program
{
    ...
    {
        string filename = @"C:\Users\ASUS\Desktop\UNI\TLA\Phase3\in\input1.json";
    ...
    public static void PHASE3(List<State> states)
    {
        string input = "";
        ...
        while (input != "%End")
        {
            ...
            {
                for(int k = 0; k < start.transitions.Count; k++)
                {
                    if (input[i].ToString() == start.transitions[k].Name)
                    {
                        //Console.WriteLine(start.Name + " " + input[i] + " " + start.transitions[k].destination.Name);
                        start = start.transitions[k].destination;
